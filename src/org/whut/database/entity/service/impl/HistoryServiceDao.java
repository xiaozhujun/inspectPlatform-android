...
public class HistoryServiceDao implements HistoryService{
	...
	public void addHistory(History history) {
		...
		db.beginTransaction();
		db.execSQL("insert into history(userId,userName,filePath,tableName,uploadFlag,inspectTime) values(?,?,?,?,?,?)",
				new Object[]{history.getUserId(),history.getUserName(),history.getFilePath(),history.getTableName(),history.getUploadFlag(),history.getInspectTime()});
		Log.i("msg", "已完成点检历史添加"+history.getUserId()+","+history.getTableName()+","+history.getUploadFlag()+","+history.getInspectTime()+","+history.getUserName()+","+history.getFilePath());
	...
	public List<Map<String,String>> queryHistory(int userId) {
		...
		while(cursor.moveToNext()){
			...
			map.put("tableName",cursor.getString(cursor.getColumnIndex("tableName")));
